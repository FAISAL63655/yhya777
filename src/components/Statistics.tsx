      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm bg-white/20 px-3 py-1 rounded-full">
            {playerStatsArray.length} لاعب مؤهل
          </span>
        </div>
        {isExpanded ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-center">عدد المباريات</th>
                  ...
                  <th className="px-4 py-3 text-center">البطاقات الصفراء</th>
                  <th className="px-4 py-3 text-center">البطاقات الحمراء</th>
                </tr>
              </thead>
              <tbody>
                {playerStatsArray.map((playerStat, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">{playerStat.totalMatches}</td>
                    <td className="px-4 py-3 text-center text-green-600">{playerStat.wins}</td>
                    <td className="px-4 py-3 text-center text-blue-600">{playerStat.draws}</td>
                    <td className="px-4 py-3 text-center text-red-600">{playerStat.losses}</td>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        {(playerStat.yellowCards > 0 || playerStat.redCards > 0) && (
                          <span className="text-xs text-red-500">
                            -{playerStat.yellowCards * 3 + playerStat.redCards * 6} نقطة
                          </span>
                        )}
                      </div>
                    </td>
                    {gameStat.gameId === games.find(g => g.type === 'FOOTBALL')?.id && (
                      <td className="px-4 py-3 text-center">
                        <span className="inline-flex items-center justify-center px-3 py-1 rounded-full font-bold bg-green-100 text-green-800">
                          {playerStat.goals}
                        </span>
                      </td>
                    )}
        ...
        if (playerMatches.length > 0) {
          ...
          playerMatches.forEach(match => {
            ...
            if (playerTeam && otherTeam) {
              ...
              if (playerMatchStats) {
                playerStats.yellowCards += playerMatchStats.yellowCards || 0;
                if (playerMatchStats.redCard) playerStats.redCards++;
                playerStats.goals += playerMatchStats.goals || 0;

                if (game.type === 'FOOTBALL') {
                  if (playerMatchStats.redCard) {
                    playerStats.points -= 6;
